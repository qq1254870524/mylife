...
- 先采集“姓名+城市州邮编”的全部分页候选；只有该搜索没有结果时才回退“姓名”搜索；
- 逐条进入详情采集年龄、生日、性别、星座，关闭可见弹层后后退一步回搜索列表；
...
CSV 每写一行都会 `flush + fsync`。输出完整保留原输入列，并只在最后追加 `生日`、`性别`、`星座`。批次正常结束后按原始源行号一次性删除已经明确输出的行；重复首列或完全相同的重复输入不会误删、漏写。技术失败行保留供下次继续。SQLite 已有结果可自动回放补齐被意外删除的实时 CSV，回放不会重复已有行。
