...
CSV 每写一行都会 `flush + fsync`。输出完整保留原输入列，并在最后追加 `生日`、`性别`、`星座`、`备注原因`；备注会写明搜索方式、结果状态、命中证据或未搜索到的原因。批次正常结束后按原始源行号一次性删除已经明确输出的行；重复首列或完全相同的重复输入不会误删、漏写。技术失败行保留供下次继续。SQLite 已有结果可自动回放补齐被意外删除的实时 CSV，回放不会重复已有行。
